...
// Dev overrides so the whole match lifecycle can be exercised quickly:
//   SPLAGOON_MATCH_SECONDS=40 npm start
// This module also runs in the browser (offline mode), where there is no
// `process`, so read the environment defensively.
const env = (typeof process !== 'undefined' && process.env) || {};
const DURATION = Number(env.SPLAGOON_MATCH_SECONDS) || MATCH.duration;
const FORCED_STAGE = env.SPLAGOON_STAGE || '';
const RESULTS_TIME = Number(env.SPLAGOON_RESULTS_SECONDS) || MATCH.resultsTime;
